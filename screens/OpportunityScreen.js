...
export default class OpportunityScreen extends Component {
  ...
  async onLoad(payload) {
    console.warn('=========onLoad : OTP=========== : o', payload);
...
onBlur(payload) {
  console.warn('=========OnBlur :OTP=========== : o', payload);
...
_keyboardDidShow = ()=> {
console.warn('_keyboardDidShow o')
  //this.setState({isFooter:!this.state.isFooter})
}

_keyboardDidHide = () => {
  this.formRef.tagArrayadd();
  console.warn('_keyboardDidHide o')
  //this.setState({isFooter:!this.state.isFooter})
}
  ...
  componentDidMount = async () => {
    ...
    if (isConnected) {
     // console.warn('componentDidMount : ', isConnected);
      ...
      console.warn('componentDidMount init formData : ', formData);
      fetch(apiBasePath, {
        ...
        .then(response => {
          console.warn('componentDidMount init formData response = : ', response)
  ...
  getTabData(selectedTab) {
    var arr = this.state.tabsData;
    for (var i = 0; i < arr.length; i++) {
      if (arr[i].tab_id == selectedTab)
        arr[i].isSelected = true;
      else
        arr[i].isSelected = false;
    }
    this.setState({ tabsData: arr });

  //  console.warn('selectedTab ; ', selectedTab);
  //  console.warn('arr : ', this.state.opportunityResultArr)
    var arr = this.state.opportunityResultArr;
    var resArr = [];
    for (var i = 0; i < arr.length; i++) {
      if (selectedTab == arr[i].tabbable_parent)
        resArr.push(arr[i]);
    }
   // console.warn('resultArr : ', resArr);
    return resArr;
    // for()

  }
  get_opportunity_fields = async () => {
   // console.warn('get_opportunity_fields')
    const isConnected = await NetworkUtils.isNetworkAvailable()

    if (isConnected) {
   //   console.warn('get_opportunity_fields 11')
      const api_token = await AsyncStorage.getItem(Constant.api_token);
...
console.warn('form data get_opportunity_fields : ' , formData);
      ...
      fetch(apiBasePath, {
        ...
        .then(response => {
          console.warn('get_opportunity_fields response : ', response)
          if (response.status == 200) {
            return response.json();
          }
        ...
        .then(responseJson => {
          ...
          if (str.error === 101) {
            ...
            this.setState({ opportunityResultArr: str.fields, tabsData: str.tabs }, () => {
              var tabs = str.tabs;
              str = str.length == 0 ? [] : str.fields;

              console.warn("Response Opp Data : ", str)
              //  this.setState({ cnt: this.state.cnt + 1 });
              if (tabs) {
                str = this.getTabData(tabs[0].tab_id)
              }
              this.setState({ isLoading: false, opportunityFieldData: str, cnt: this.state.cnt + 1 }, () => {
            //    console.warn('********** tab repson se ********** : ', this.state.tabsData, 'asdasdsdds')

                this.modifyOpportunityFieldData()
  ...
  getFormResponses = async () => {
    ...
    console.warn(' getFormResponses  : ', response);
    var o = {};
    var a
    ...
    console.warn("opportunioty ress ... : ", o)
  ...
  modifyOpportunityFieldData() {
    ...
    if (this.state.opportunityFieldData.length == 0 || this.state.opportunityFieldData == undefined || this.state.opportunityFieldData == "null") {
     ...
     // console.warn("item : ", item)
      ...
      // console.warn('opportunity field data ===== : ', item);
      // for (var i = 0; i < item.length; i++) {
      //   console.warn('item result 666 :: ' , item[i])
      //   if (item[i].type == 'tagsinput'){
      //     var obj = item[i].value
      //    // var arr = obj.split(',');
      //     console.warn('result of tags array asdasdsdsad: ',obj)

      //     for(var j in obj){
      //         console.warn('11111122223333 : ' , j , obj[j]);
      //     }
      //   }
  ...
  getTabListData() {
    return this.state.tabsData.map((item, index) => {
      return (
        <TouchableOpacity style={{marginRight:10, width: 130, alignItems: 'center', backgroundColor: item.isSelected == true ? Colors.green_start : Colors.primaryColor, height: 42, borderRadius: 42 / 2, }}
          onPress={() => { this.setState({ opportunityFieldData: this.getTabData(item.tab_id), cnt: this.state.cnt + 1 }, () => { this.modifyOpportunityFieldData() }) }}>

          <Text style={[styles.text_style], { fontSize: 14, fontWeight: '600', marginTop: 12, justifyContent: 'center', color: '#fff', textAlignVertical: 'center' }}>{item.tab_name}</Text>
  ...
  render() {
    ...
    return (
      ...
      <KeyboardAwareScrollView style={styles.main_container}
       ...
       // scrollEnabled={this.state.isModalVisible}
       
      >

<NavigationEvents onDidFocus={(payload) => { this.onLoad(payload) }} onDidBlur={(payload) => { this.onBlur(payload) }} />
        ...
        <View style={{ backgroundColor: Colors.white_color, marginTop: 10, paddingStart: 16, paddingEnd: 16 ,paddingBottom:10}}>
          ...
          <View style={{ flexDirection: 'row', flex: 1, }}>
            ...
            <TouchableOpacity style={{ marginTop: 14, width: '36%', alignItems: 'center', backgroundColor: Colors.primaryColor, height: 42, borderRadius: 42 / 2 }}
              activeOpacity={1} onPress={() => {this.props.navigation.setParams({ isModalVisible: true }), this.setState({ displayMessage: false, isModalVisible: true }, () => { this.formRef.ResetForm(), this.get_opportunity_fields() }) }}>
...
<View style={{ backgroundColor: Colors.white_color, marginTop: 10, paddingStart: 16, paddingEnd: 16,height:'100%'}}>

          <View style={styles.formcontainer} key={this.state.cnt}>
            ...
            {this.state.tabsData ?
              ...
              //     <TouchableOpacity style={{ width: '95%', alignItems: 'center', backgroundColor: item.isSelected == true ?Colors.green_start:Colors.primaryColor, height: 42, borderRadius: 42 / 2, }}
              //       onPress={() => { this.setState({opportunityFieldData:this.getTabData(item.tab_id),cnt:this.state.cnt+1},()=>{ this.modifyOpportunityFieldData()})}}>
              ...
              //     style={{ borderWidth: 2, width: '100%' }}>{this.getTabListData()}</ScrollView>
              // </View>
...
<ScrollView horizontal={true} showsHorizontalScrollIndicator={false} style={{width:'100%'}}>
  ...
  {this.getTabListData()}
        ...
        {(this.state.isModalVisible) ?
          ...
          <View style={{ position: 'absolute', width: '100%', height: heightPercentageToDP(100), }} activeOpacity={0}  >


            <View style={{ height: '89%', }} >
              <TouchableOpacity style={{ height: '40%', backgroundColor: 'black', opacity: 0.3 }} onPress={() => { this.props.navigation.setParams({ isModalVisible: false }), this.setState({ isModalVisible: false }) }} ></TouchableOpacity>
              <View style={{ padding: 10, backgroundColor: '#ffffff', height: '60%' }} activeOpacity={0} >

                <View style={styles.dropdown_view, { marginTop: 5 }}>
                  <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Text style={styles.text_style2}>Add Opportunity</Text>
                    <TouchableOpacity onPress={() => { this.props.navigation.setParams({ isModalVisible: false }), this.setState({ isModalVisible: false }) }}>
                  ...
                  {(this.state.selected_Company_opportunity && this.state.select_company_Opportunity != "Select Opportunity") ?
                    ...
                    <TouchableOpacity style={{ justifyContent: 'center', alignSelf: 'flex-end', width: '18%', height: 42, marginTop: 20, backgroundColor: Colors.primaryColor, borderRadius: 42 / 2 }}
                      onPress={() => {this.props.navigation.setParams({ isModalVisible: false }), this.setState({ selected_Company_opportunity: false, isModalVisible: false }, () => { this.add_Contact_Opportunities() }) }}>
