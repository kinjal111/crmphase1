...
this.setState({printSelecteddate:''});
    var months = monthNames[new Date().getMonth()]; //Current Month
    var year = new Date().getFullYear(); //Current Year
    this.setState({});
    const { navigation } = this.props;
    var apptDt = navigation.getParam('Apptdate', null);
    this.setState({currentDate: apptDt == null?new Date():apptDt});
    ...
    .then(json => {
      ...
      console.warn('user login date formate : ' , dtRes , this.state.appointmentDTformat.date_format);

    
   
      console.warn('appt dt ***************** 11: ' , apptDt);
        if(apptDt == null)
          apptDt = dtRes;
          console.warn('appt dt ***************** 22: ' , apptDt);
         
        this.setState({
          //Setting the value of the date time
          date: apptDt,//months + ' ' + date1 + ', ' + year + ' ',
          selectedDate:apptDt,
          month: months,
          year: year,
        }, () => { 
          
          //this.highlightSelectDay(); this.SetDisplayDateFormat(this.state.date);
...
//console.warn('fetchInintialData')
    ...
    console.warn("FormData Calader",formData);
    fetch(apiBasePath, {
      method: 'POST',
      body: formData
    })
...
//console.warn('fetchApointmentMeetAgentData')
    await AsyncStorage.setItem(Constant.selectedCompany, this.state.select_company);


    const api_token = await AsyncStorage.getItem(Constant.api_token);
    let formData = new FormData();
//console.warn('select company : ', this.state.select_company)
    formData.append('api_key', 'b4bc8f195a1c926b184f33a466bbc837689b33fe');
    formData.append('api_token', api_token);
  ...
  fetchCalendarData = async () => {
   // console.warn('fetchCalendarData')
    const api_token = await AsyncStorage.getItem(Constant.api_token);
    let formData = new FormData();
...
//console.warn('modifyCompanyAccessKey')
  ...
  modifyAgentKey() {
//console.warn('modifyAgentKey')
    if (this.state.MeetAgentData == undefined || this.state.MeetAgentData.length == 0 || this.state.MeetAgentData == 'null') {

    }
    else {
      const {LoginUserDetail} = this.state
      let logedInUser = LoginUserDetail && LoginUserDetail.name 
      var item = this.state.MeetAgentData;
      let findIndex = 0; 
      item.map((val,index) => {
        val.value = val.id;
        val.label = val.name;
        if(logedInUser === val.name ){
          findIndex = index
        } 
        delete val.name
        delete val.id
      })
       this.setState({ MeetAgentData: item, MeetAgentState: false, select_Agent: item[findIndex].value }, () => {  this.fetchCalendarData() })
    }
  }
...
//console.warn('createMarkData')
    if (this.state.calendarData == 'null' || this.state.calendarData == undefined || this.state.calendarData.length == 0) {
      ...
      // console.log("selected date 4$$$$$$$$$$$$$$$$ : " ,JSON.stringify(obj))
      this.setState({ markedDate: obj, selectedDate: this.state.selectedDate, markData: true },()=>{
//var dt = new Date(this.state.selectDate);
var dt = moment(this.state.selectedDate, "YYYY-MM-DD");
//let momentObj = moment(this.state.selectedDate, (this.state.appointmentDTformat.date_format).toUpperCase())

console.warn('Date : ' ,this.state.selectedDate ,  dt)
//console.warn(' day  : ' , dt.month );
...
console.warn("====== : " , date + '-' + month + '-' + year , " - " , time);

        var obj = {"dateString": this.state.selectedDate, "day": date, "month": month, "timestamp": time, "year": year}
        this.highlightSelectDay(); this.SetDisplayDateFormat(obj);
      });
...
console.warn("==== refresh ========== ")
  ...
  SetDisplayDateFormat = (Date) => {
    ...
    this.setState({ printSelecteddate: monthNames[Date.month - 1] + ' ' + Date.day + ', ' + Date.year },()=>{
      console.warn('printSelecteddate  : ' ,Date ,  this.state.printSelecteddate) ;
    })
  }
  ...
  highlightSelectDay = () => {
    ...
    if (this.state.markedDate.length == 0 || this.state.markedDate === undefined || this.state.markedDate === 'null') {
      ...
      for(i in item){
      //  console.warn("------------------------------------------------------------------ " , i , this.state.selectedDate);
        if( i == this.state.selectedDate){
        //  console.warn("================  i : " , i , item);
          isAvailable = true;
        }
         
      }
     if(isAvailable == false){
       item[this.state.selectedDate] = {"dots":[],marked:true,selected:true};
    //   Object.assign(markedDates, { [data.date]: { selected: your state if selected } }); });
  //     this.setState({markedDate: {}}, function(){ this.setState({markedDate: item})}) 
     this.setState({markedDate : item})
    //  this.forceUpdate();
     }
     // var obj = new Object();
     // obj[this.state.selectedDate] = {"dots":[],marked:true,selected:false};
    //  console.warn('obj : ' , obj);
  //  item[this.state.selectedDate] = {"dots":[],marked:true,selected:false};
    // item.append(obj);
     // item.push(obj);
...
//console.warn('marked date : ' ,)
    ...
    //   var obj = new Object();
      // {"dots": [{"color": "#428bca", "selectedDotColor": "#428bca"}], "marked": true, "selected": false}
  //    obj[this.state.selectedDate] = {"dots":[{"color": "#428bca", "selectedDotColor": "#428bca"}],marked:true,selected:false};
    //  console.warn('obj : ' , obj);
    ...
    //  data_ = data_ +obj;
      ...
      //  markedDate
       ...
       this.setState({cnt:this.state.cnt+1})
          console.warn('item ====:: ',this.state.cnt, this.state.markedDate)});
  ...
  updateCalendarDate() {
console.warn('=================  updateCalendarDate : ', calendarDate, ' :: ' , calendarDate.format('MMMM-DD-YYYY'))
    this.setState({
      ...
      this.setState({ month: monthNames[Month1 - 1] }, () => { 
        ...
        console.warn('update calendar Date : ' , this.state.month )
      })
    });
...
//console.warn("note123",value);
    return (
     <Text>{value}</Text>
      //  <HTML  style={[styles.bBencoolen]}  html={value}/>
    )
  }
...
<View key={this.state.cnt}>
          <CalendarList
            ...
            onDayPress={(date) => this.setState({ selectedDate: date.dateString }, () => {
              console.warn('date string :  ',date , (new Date())),
          ...
          <View style={{ backgroundColor: Colors.bg_color }}>

            {
              ...
              (this.state.calendarData) ?
               ...
               //   console.warn('selected Date : ' , this.state.selectedDate + " : " + item.date)
                  if (this.state.selectedDate == item.date) {
                //   console.warn("Item...",item);
                    return (<TouchableOpacity activeOpacity={1}

                      onPress={() => { this.props.navigation.navigate('Details', { Detail: item , Screen : 'Calendar',Apptdate:this.state.selectedDate}) }} >
