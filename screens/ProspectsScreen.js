...
export default class ProspectsScreen extends React.PureComponent {
 ...
 console.warn('navigations ..... : ' , navigation);
  return {


    headerBackground: () => <HeaderBackground />,

    headerTitle: () => <Text style={textHeader.header}>{params.categName}</Text>,
  ...
  componentDidMount = async () => {
    ...
    let categ = this.props.navigation.getParam('categ');
    if(categ == 'client')
      categ = 'Client';
    else if(categ == 'prospect')
      categ = 'Prospect'
    else if(categ == 'lead')
      categ = 'Lead'

    this.props.navigation.setParams({ categName: categ }), 
 ...
 getContactNote = async () => {
  // this.getContactData();
  //console.warn("GetContact.... ", this.state.ProspectData.length, "Page Come....",this.state.page)
  const isConnected = await NetworkUtils.isNetworkAvailable()
// this.toggleAllNote();
  if (isConnected) {
   ...
   // console.warn("FormData124...", formData);
    ...
    if (this.mounted) {
      ...
      fetch(apiBasePath, {
        ...
        .then(responseJson => {
        //  console.warn("Response Note",responseJson);
          str = responseJson
          // this.toggleAllNote()  
          if (str === [] ) {
            ...
            this.setState({
              ...
              if(str.length ==0){
                ...
                this.setState({
                  ...
                  () => {
                    // console.warn("Page Value::::",this.state.page)
                   
                    this.getContactNote();
                    // console.warn("Page After ::::",this.state.page)
                  }
                );
              }
  ...
  //  console.warn('conversation data : ', this.state.ConversationData)
    ...
    this.setState({ ConversationData: item, ConversationState: true, coversation: item[1].value, conversation: 'View All Conversation ', conversationtoShow: 'All' ,page:1}, () => { this.getContactNote() })
  }
  ...
  openDownloadedAttachment = async (item, filename) => {  
    if (this.state.isSpinner == false) {
     this.setState({ isSpinner: true });
     const localFile = `${RNFS.DocumentDirectoryPath}/ ` + filename; 
     this.setState({ isSpinner: false },()=>FileViewer.open(localFile)) 
   } 
 }

  openIosPhoto = async (chec,ext) => {  
    if (this.state.isSpinner == false) {
    this.setState({ isSpinner: true });
     const hash = chec.substring(5, 41); 
    let path = `assets-library://asset/asset.${ext}?id=${hash}&ext=${ext}`; 
    let dest = `${RNFS.TemporaryDirectoryPath}${Math.random().toString(36).substring(7)}.jpg`;
    await RNFS.copyAssetsFileIOS(path, dest, 0, 0).then(()=>{ 
      FileViewer.open(dest, {
        onDismiss: () => this.setState({ isSpinner: false })
      });
    }).catch(
      this.setState({ isSpinner: false })
    ); 
    ...
    try {
      const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE,
        {
          ...
          message:"Write Permission For Save The File",
  ...
  downloadAttachment = async (item, filename) => {
    console.warn('downlaod attachement',item, filename)
 ...
 */
    ...
    var ext = arr[4];
    let isPhoto = ext == 'JPG' || ext == 'PNG' || ext == 'JPEG' || ext == 'GIF' ? true : false
      ...
      RNFS.downloadFile(options).promise
        .then(() => { 
          if(isPhoto){
            CameraRoll.saveImageWithTag(localFile)
              .then((path)=>{
                // console.log('PAth => ',path)
                RNFS.unlink(localFile)
                showMessage({
                  description: "Click Here to View File",
                  message:`Attachment is Downloaded Successfully.`,
                  type: "success",
                  duration:3000,
                  onPress:()=>{this.openIosPhoto(path,ext)},
                 });
                }   
              ) 
            ...
            showMessage({
              ...
              type: "success",
              duration:3000,
              onPress:()=>{this.openDownloadedAttachment(item,filename)},
             });
          }
  ...
  toggleAllNote() {
    ...
    function custom_sort(a, b) {
     ...
     var dt1 = moment(a.date_created, "YYYY-MM-DD hh:mm:ss");
     var time1 = new Date(dt1).getTime();
     var dt2 = moment(b.date_created, "YYYY-MM-DD hh:mm:ss");
     var time2 = new Date(dt2).getTime();
  //    console.warn('time :  ' , time1 , ' --- ' , time2)
      var dateA = new Date(a.date_created).getTime();
      var dateB = new Date(b.date_created).getTime();
  //    console.warn('custome Sort : ' , a.date_created , ' --- ' , b.date_created)
    ...
    if (this.state.toggleSingleNote) {
      ...
      //  console.warn('toggle data Note....... : ', item);
  ...
  _handleLoadMore = () => {
    ...
    if (this.state.ShowFooter) { }
      ...
      this.setState({
        ...
        page: this.state.page + 1, ShowFooter: false
  ...
  handleOnEndReached() {
    // console.warn('handleOnEndReached')
  }
  ...
  getExtention(name, path) {
    ...
    if (ext == 'JPG' || ext == 'PNG' || ext == 'JPEG' || ext == 'GIF') {
      ...
      return (
        <TouchableOpacity onPress={() => { this.openAttachment(filePath, name) }} style={{ width: '26%', paddingTop:10 ,borderColor: '#ccc' }}>
  ...
  getExtentionFollowUp(name, path) {
    ...
    if (ext == 'JPG' || ext == 'PNG' || ext == 'JPEG' || ext == 'GIF') {
      ...
      return (
        ...
        <TouchableOpacity onPress={() => { this.openAttachment(filePath, name) }} style={{ width: '26%', marginTop: 10 }}>
          <Image source={{ uri: filePath }} style={[{
            width: 80, height: 80,
          }]} resizeMode="stretch" />
        </TouchableOpacity>
              ...
              {(item.category === 'appointment') ?
                ...
                <View style={{ backgroundColor: '#ffffff' }}>
                  ...
                  <TouchableOpacity style={{ flexDirection: 'row', marginHorizontal: 16, marginBottom: 10 }}
                    ...
                    <View style={[styles.top_image_style, { marginTop: 10, alignItems: 'flex-end', tintColor: Colors.primaryColor, justifyContent: 'center' }]} />
  ...
  render() {
    ...
    return (
      ...
      refreshControl={
        <RefreshControl refreshing={this.state.isLoading} onRefresh={()=> this.setState({isLoading:true},()=>this.getContactData()) } />
      }  
      >
        <Spinner
        ...
        <View style={styles.container}>
          ...
          <NavigationEvents
            onDidFocus={() => this.setState({ page: 1 }, () => { this.getContactNote(), this.getContactData() })}
